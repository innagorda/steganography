	...
	for (int i = 0; i < hide_image.rows; i++)
		for (int j = 0; j < hide_image.cols; j++) {
			for (int bit = 0; bit < 8; bit++) {
				...
				auto mask = get_mask<int>(7 - bit, 1);

				for (int channel = 0; channel < 3; channel++) {

					curr_bit = (hide_image.at<cv::Vec3b>(i, j)[channel] & mask) >> 7 - bit;
						//std::cout << curr_bit;
			...
			if ((i == 0) && (j == 0))
				std::cout << (int)hide_image.at<cv::Vec3b>(0, 0)[0] << " " << (int)hide_image.at<cv::Vec3b>(0,0)[1]	<< " " << (int)hide_image.at<cv::Vec3b>(0,0)[2] << std::endl
				<< (std::bitset<8>)hide_image.at<cv::Vec3b>(0, 0)[0] << " " << (std::bitset<8>)hide_image.at<cv::Vec3b>(0, 0)[1] << " " << (std::bitset<8>)hide_image.at<cv::Vec3b>(0, 0)[2] << std::endl;
	}
	return image;
}
...
cv::Mat get_image(cv::Mat image) {
	...
	hide_image = cv::Mat(hide_image_rows, hide_image_cols, CV_8UC3);
...
int main() {
	...
	cv::namedWindow("imgHidden", CV_WINDOW_AUTOSIZE);
	//cv::imshow("imgOriginal", imgOriginal);     // show windows
	//cv::imshow("imgChanged", imgChanged);
	cv::imshow("imgFound", imgFound);
	cv::imshow("imgHidden", imgHidden);
